...
let snoElement = document.getElementById("sno");
...
//select裡面的option被運用 當區域被選擇的時候 
//顯示資料有沒有在列表裡 如果有顯示"有這個區域"
snoElement.addEventListener("change", (event) => {
    let selectedIndex = snoElement.selectedIndex;
    console.log("selectedIndex:"+ selectedIndex)
    selectedValue = snoElement.options[selectedIndex].value;
    console.log("selectedValue:"+ selectedValue)
    if (sno_array.includes(selectedValue)) {
        // console.log('有這個區域')
        // console.log(`行政區:${selectedValue}`)
        areaNameElement.innerText = selectedValue
        let trHTML = "" //行政區內的html資訊
...
//把讀取的資料整理並存入到html select裡 for sno
function snoreqListener() {
    youbikedata = JSON.parse(this.responseText);

    for (const youbike of youbikedata) {
        sno_array.push(youbike.sno);
    }
    sno_array = [...new Set(sno_array)]//Set功能不會重複加入資料
    console.log(sno_array)//print 資料到主控台 
    let optionElement = document.createElement("option");
    optionElement.textContent = '請選擇站點序號'
    snoElement.appendChild(optionElement);

    //將讀取到的區名加入到select中    
    for (const sno of sno_array) {
        let optionElement = document.createElement("option");
        optionElement.textContent = sno;
        optionElement.setAttribute('value', sno)
        snoElement.appendChild(optionElement);
    }
}
...
const windowload = (event) => {
    ...
    req.addEventListener("load", snoreqListener);
